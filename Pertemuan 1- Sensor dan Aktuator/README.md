...
- Percobaan 2 (Kendali Aktuator): Bercerita dari percobaan pertama dengan menambahkan Relay/LED sebagai aktuator. Mikrokontroler diprogram untuk mengambil           keputusan (menyalakan atau mematikan aktuator) secara otomatis berdasarkan nilai suhu yang terbaca dibandingkan dengan nilai ambang batas (threshold ) yang        telah ditetapkan sebesar 30 derajat Celsius.
-----
# 2. Pustaka dan Ketergantungan
1. Perpustakaan sensor DHT (oleh Adafruit)
2. Papan esp8266 esp32
3. Driver CP20X untuk esp8266
-----
# 3. Penjelasan Kode & Percabangan
## Percobaan 1:
Penjelasan Kode:
- `#include <DHT.h>`: Mengimpor pustaka DHT agar fungsi-fungsi sensor pembacaan dapat digunakan.
...
- `DHT dht(DHTPIN, DHTTYPE);`: Membuat objek bernama dht dengan parameter pin dan tipe sensor yang telah ditentukan.
...
- `float kelembaban = dht.readHumidity();`: Memanggil fungsi untuk membaca kelembaban.

- `float suhu = dht.readTemperature();`: Memanggil fungsi untuk membaca suhu.

- `delay(2000);`: Menghentikan program selama 2000 milidetik (2 detik).
...
```
Logika if-else ini bertugas memvalidasi data. Jika hasil pembacaan suhu ATAU (||) kelembaban bernilai NaN (Not a Number), maka program akan mencetak pesan "Gagal". Jika data valid (bukan NaN), program masuk ke blok lain untuk mencetak angka suhu dan kelembaban ke layar.

# Percobaan 2: Relay Aktuator Kendali
...
- `const float suhuThreshold = 30.0;` : Membuat variabel konstan bertipe desimal senilai 30 derajat celsius sebagai batas acuan suhu untuk memicu aktuator.
...
- `float sumKelembaban = 0;`: Mendeklarasikan variabel jumlahKelembaban bertipe float dengan nilai awal 0 untuk menampung total akumulasi nilai kelembaban.
...
- `} else if (suhu < suhuBatasBawah) {`: Kondisi alternatif (histerisis). Jika suhu tidak lebih dari batas atas, program memeriksa apakah suhu turun lebih kecil     dari 28.0°C.
...
Penerapan 1
<img width="447" height="546" alt="Screenshot 2026-09-03 211040" src="https://github.com/user-attachments/assets/a0e52f5d-ff0f-459d-8904-9846afd8a7e6" />

Penerapan 2
<img width="762" height="556" alt="Screenshot 2026-09-03 211401" src="https://github.com/user-attachments/assets/add62804-ae35-4749-ac92-cae05287f586" />
